<?php
	...
	$statement->execute();
	$statement = $pdo->prepare("INSERT INTO action (user_id, post_id, message) VALUES (?, ?, ?)");
	$statement->execute([$user_id, $post_id, $message]);
	$stmt = $pdo->prepare("SELECT id FROM comments WHERE user_id = '$user_id' AND post_id = '$post_id' AND message = '$message' LIMIT 1;");
    $stmt->execute();
